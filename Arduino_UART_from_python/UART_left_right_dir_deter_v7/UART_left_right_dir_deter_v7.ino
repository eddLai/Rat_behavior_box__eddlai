...
void setup() {
  ...
  Serial.println("=========================================");
  Serial.println(" System Ready! Please select a mode: ");
  Serial.println(" Enter '1' : Camera detection mode");
  Serial.println(" Enter '2' : Standup (Auto Interval) mode (Right Door)");
  Serial.println(" Enter '3' : Manual Control mode");
  Serial.println(" Enter '4' : Drain mode");
  Serial.println("=========================================");
  ...
  if (receivedChar == '1') {
    mode = 1;
    Serial.println("-> [Mode 1] Camera detection mode selected");

  } else if (receivedChar == '2') {
    mode = 2;
    Serial.println("-> [Mode 2] Standup mode selected");
    Serial.println("    Random Time Interval (Auto), open only Right Door");

  } else if (receivedChar == '3') {
    mode = 3;
    Serial.println("-> [Mode 3] Manual Control mode selected");
    Serial.println("   Tutorial: Enter '1' to OPEN/BLINK, '0' to CLOSE/STOP)");
  
  } else if (receivedChar == '4') {
    mode = 4;
    Serial.println("-> [Mode 4] Drain mode selected");
...
void processResponse(char response){
  ...
  if (response == 'D') {
    ...
    Serial.println("Sudden stop received: Doors closed");
    blinking = false;
    resetState();
    return;
  }
  ...
  if (isObstacle_M) {
    if (response == 'R' && ans == 2 && !isObstacle_R) {
      ...
      // 平滑開啟右門
      Serial.println("right door opening...");
      ...
      // 平滑關閉右門
      Serial.println("right door closing...");
      moveServoSmooth(reward_R, currentPosR, posClose);
      currentPosR = posClose;
      ...
      // 平滑關閉左門
      Serial.println("left door closing...");
      moveServoSmooth(reward_L, currentPosL, posClose);
      currentPosL = posClose;
...
void stand_up() {
  // 1. 重新連接右邊馬達(因為在 loop 切換模式時被 detach 了)
  reward_R.attach(9);

  // 2. 平滑開啟右門
  // digitalWrite(pumpPin, LOW);  // 啟動水泵
  Serial.println("Stand-up: Right door opening...");
  moveServoSmooth(reward_R, currentPosR, posOpen);
  currentPosR = posOpen;

  // 3. 閃爍LED燈, 持續6秒, 每次閃爍間隔500毫秒 (這 6 秒期間，右門會維持開啟狀態給水)
  Serial.println("Blink every 500ms for 6sec");
  blinkLed(6, 500);            

  // 4. 平滑關閉右門
  Serial.println("Stand-up: Right door closing...");
  moveServoSmooth(reward_R, currentPosR, posClose);
  currentPosR = posClose;
  ...
  Serial.print("Stand-up: Waiting for next trial (");
...
void manual_mode() {
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '1') {
      ...
      if (!isManualActive) {
        Serial.println("Manual Switch: Right door OPEN & Blinking START");
        isManualActive = true;
      ...
      if (isManualActive) {
        Serial.println("Manual Switch: Right door CLOSE & Blinking STOP");
...
void drain_mode() {
  ...
  if (!menuPrinted) {
    Serial.println("=========================================");
    Serial.println(" Drain mode activated.");
    Serial.println(" Enter 'O' to OPEN doors and start draining.");
    Serial.println(" Enter 'C' to CLOSE doors and detach motors.");
    Serial.println("=========================================");
  ...
  if (Serial.available() > 0) {
    ...
    if (cmd == 'O' || cmd == 'o') {
      ...
      Serial.println("-> Doors OPENING... Water is draining.");
      moveServoSmooth(reward_L, currentPosL, posOpen);
      moveServoSmooth(reward_R, currentPosR, posOpen);
      currentPosL = posOpen;
      currentPosR = posOpen;
    ...
    // 輸入 'C' 或 'c' 關門
      ...
      reward_L.detach();
      reward_R.detach();
      Serial.println("-> Doors are closed and motors detached. Safe to idle.");
    } 
    // 輸入其他無效字元
    else {
