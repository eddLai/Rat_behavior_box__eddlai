...
sensor_D=17
sensor_MD=27
#since pin 13 and 15 need the root user
GPIO.setmode(GPIO.BCM)
GPIO.setwarnings(False)
GPIO.setup(sensor_M, GPIO.OUT)
GPIO.setup(sensor_L, GPIO.OUT)
GPIO.setup(sensor_R, GPIO.OUT)
GPIO.setup(sensor_D, GPIO.OUT)
GPIO.setup(sensor_MD, GPIO.OUT)
...
def pin_output_ifs(x, y, distance_from_target):
    ...
    # down
        ...
        GPIO.output(sensor_MD, GPIO.HIGH)
        print("MD")
    else:
        # print("未經過目標區域")
        pass
...
while True:
    ...
    GPIO.output(sensor_M, GPIO.LOW)
    GPIO.output(sensor_L, GPIO.LOW)
    GPIO.output(sensor_R, GPIO.LOW)
    GPIO.output(sensor_MD, GPIO.LOW)
    GPIO.output(sensor_D, GPIO.LOW)
